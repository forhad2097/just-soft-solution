  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl md:text-4xl font-bold tracking-tight">Services</h1>
          <p className="mt-2 text-sm text-[var(--muted-foreground)]">
            {services.length} total · {services.filter((s) => s.status === "published").length} published
          </p>
        </div>
        <Link
      ...
      {saved ? (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">
          Service <code>{saved}</code> saved successfully.
        </div>
      ) : null}
      {deleted ? (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
          Service <code>{deleted}</code> deleted.
        </div>
      ) : null}
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {services.map((s) => (
              <tr key={s.slug} className="border-b border-[var(--border)] last:border-0">
